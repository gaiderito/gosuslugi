...
<header>
    ...
    <div class="big">
        ...
        <?php   $fio = $_SESSION['user']['fio'];
                $sql = mysqli_query($conn, "SELECT * FROM `reports` WHERE `fio` = '$fio' ");
                if (mysqli_num_rows($sql)>0){
                    ...
                    echo('<div class="myreport">
                            ...
                            <div><p class="stat">Статус: '.$user['status'].'</p><form action="config/del.php" method="post"><input type="hidden" name="id" value="'.$user['id'].'"><button type="submit">Удалить</button></form></div>
                            </div>
                ');}
        }else{
            echo('<div class="nothing">
            <h2>У вас нет ни одной жалобы</h2>
